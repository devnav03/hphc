								...
								@if(!empty($data))
									...
									<tr>
										...
										<td>										
											...
											$leave_user_id = $data_rec->user_id;
											
											$search_tour_leave = 'TOUR';

											if(strpos(strtoupper($data_rec->leave_type_name), $search_tour_leave) != false && $leave_user_id != $login_user_id){ ?>
										...
										<td>{{$data_rec->leave_type_name}}</td>
										<td>@if($data_rec->time_duration==null){{'NA'}} @else {{$data_rec->time_duration}} @endif</td>
										<td>
											@if($data_rec->status=='Pending')
											<a href="javascript:void(0)" onclick="checkAuthorityLeave('{{$data_rec->id}}')" class="btn-xs btn btn-primary inds'{{$data_rec->id}}'"> Pending</a>
		...
		function checkAuthorityLeave(id){
            ...
            $.ajax({
                ...
                success:function(xhr){
					spinner.hide();
					if(xhr.data.result){
						alert("Hi you are not eligible to approve your own leave. Please contact to Administrator");
        ...
        $(document).on('click','.status_checks',function(){
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
                        url: "{{url('ajax/employee-leave-status')}}",
                        type: "POST",
...
<script>
  ...
  $(document).on('click','.showForwardModal',function(){
	...
	////////////////////// Code for Tour leave end date change /////////////////////////////
	...
	function updateTourLeaveEndDate(leave_id, end_date, duration, leave_user_id){
		...
		$.ajax({
			headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
			url: "{{url('update-tour-leave-end-date')}}",
			type: "POST",
