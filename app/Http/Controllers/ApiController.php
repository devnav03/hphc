...
class ApiController extends Controller
{
    ...
    private function getAddress($latitude, $longitude) {
        $apiKey = 'AIzaSyCmmPEzXkiZrX81UJ3SP0C__THCJdOrnaI';
    ...
    public function AttendanceHistory(Request $request){
        ...
        if($user){
            ...
            $date = \Carbon\Carbon::today()->subDays($request->days);
            $attendances = EmpAttendance::where('user_id', $user_id)->where('created_at','>=',$date)->select('in_time', 'out_time', 'total_time', 'in_image', 'out_image', 'created_at', 'in_latitude', 'in_longitude', 'out_latitude', 'out_longitude')->orderBy('id', 'desc')->get();
            } else {

            $attendances = EmpAttendance::where('user_id', $user_id)->select('in_time', 'out_time', 'total_time', 'in_image', 'out_image', 'created_at', 'in_latitude', 'in_longitude', 'out_latitude', 'out_longitude')->orderBy('id', 'desc')->get();
            }
            if($attendances){
                $url = route('get-started');
                foreach ($attendances as $attendance) {
                    ...
                    $slide['in_address'] = '';
                    $slide['out_address'] = '';
                    if($attendance->in_longitude){
                        $address = $this->getAddress($attendance->in_latitude, $attendance->in_longitude);
                        $slide['in_address'] = $address;
                    }

                    if($attendance->out_latitude){
                        $address = $this->getAddress($attendance->out_latitude, $attendance->out_longitude);
                        $slide['out_address'] = $address;
                    }
    ...
    public function MarkAttendance(Request $request){
        ...
        if(!empty($user)){
            ...
            if(!empty($attendance[0])){
                ...
                $emp_info = EmployeeInfo::where('user_id', $user_id)->where('employee_code', '!=', NULL)->select('office_id')->first();
                $ofc = OfficeMaster::where('id', $emp_info->office_id)->select('latlong', 'longitude')->first();

                $latitudeFrom = @$ofc->latlong;
                $longitudeFrom = @$ofc->longitude;
                ...
                $emp_attendance->out_distance = $miles;
                $emp_attendance->save();
                ...
                $emp_attendance->in_distance = $miles;
