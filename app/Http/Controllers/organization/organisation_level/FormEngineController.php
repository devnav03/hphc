...
class FormEngineController extends Controller{
    ...
    public function SaveForm(Request $request){
        ...
        $refer_code = $request->employee_code;
        		...
        		$departments = DepartmentMaster::select('department_name')->where('id',$dept_id)->first();
           // echo "<pre>"; print_r($departments); echo "</pre>"; 
            ...
            $all_data['department'] = $departments->department_name;
            $all_data['shift_name'] = 'DayShift';
       ...
       // dd($res);
        ...
        if(!empty($formenginecat)){
			...
			if($request->created_by){
				return redirect('add-user-employeess/'.$users->id.'?page='.$formenginecat)->with('success','Saved successfuly');
			}else{
			...
			if($request->created_by){
				...
				return redirect('add-user-employeess')->with('success','Saved successfuly');
			}else{
    ...
    function sendFaceCheck($all_data,$visitor_id,$image_data){
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'http://10.146.19.59:8000/api/public/employee/add',
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => '',
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,
          ...
          CURLOPT_CUSTOMREQUEST => 'POST',
          CURLOPT_POSTFIELDS =>'{
          "office_name": "High Court Himachal Pradesh",
          "department_name": "'.$all_data['department'].'",
          "shift_name": "'.$all_data['shift_name'].'",
    ...
    $curl = curl_init();
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'http://10.146.19.59:8000/api/public/employee/allot',
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => '',
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
    ...
    public function getDeviceAllocateUser($user_id){
        ...
        $device_details[0]['device_name'] = '7 Inch 1815600';
        $device_details[0]['office_name'] = 'High Court Himachal Pradesh';
        
        $device_details[1]['device_name'] = '7 Inch 1810571';
        $device_details[1]['office_name'] = 'High Court Himachal Pradesh';
