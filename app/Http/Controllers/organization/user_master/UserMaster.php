<?php
	...
	namespace App\Http\Controllers\organization\user_master;
	...
	class UserMaster extends Controller
	{
		...
		*/
		...
		public function user_role(Request $request){
			...
			if(!empty($request->office_name)){
				...
				if(empty($office_check)){
					...
					return redirect('user-role')->with('error','Office Name and Address Already Exist');
				}
			}
			return view('organization.user_master.add_role',compact('organisation','update','country','state','city'));
		}
		...
		public function add_user_role(Request $request){
			...
			$role = new Role;
			$role->name = $request['office_name'];
			$role->slug = $slug;
			$role->description = $request['address'];
			$role->save();
			return redirect('user-role')->with('success','Saved successfuly');
		...
		public function edit_user_role(Request $request, $id){
			...
			if(!empty($request->update_id)){
				if($request->update_id>0){
					...
					$role = Role::where('id',$request->update_id)->first();
					$role->name = $request['office_name'];
					$role->slug = $slug;
					$role->description = $request['address'];
					$role->save();
					return redirect('user-role')->with('success','Updated successfuly');
				}
				else{
					return redirect('user-role')->with('error','Something went wrong!');
				}
			}
			return view('organization.user_master.edit_user_role',compact('roles','organisation','id'));
		}
		
		
		public function delete_user_role(Request $request, $id){
			Role::where('id',$id)->delete();
			return redirect('user-role')->with('success', 'Deleted successfully');  
		}
		...
		public function user_list(Request $request){
			...
			return view('organization.user_master.user_list',compact('organisation'));
			
		}
		public function edit_user_list(Request $request, $id){
			
			$role = Role::select(['id','name'])->orderBy('name', 'ASC')->get();
			$role_ids = RoleUser::select(['role_id'])->where('user_id',$id)->first();
			$user_id = Auth::user()->id;
			$organisation = $this->GetOrganisation($user_id);
			if(!empty($request->update_id)){
				if($request->update_id>0){
					$roleuser = RoleUser::where('user_id',$request->update_id)->first();
					if(isset($roleuser['user_id']) && !empty($roleuser['user_id'])){
						$roleuser->user_id = $request->update_id;
					...
					return redirect('user-list')->with('success','Updated successfuly');
				}
				else{
					return redirect('user-list')->with('error','Something went wrong!');
				}
			}
			return view('organization.user_master.edit_user_list',compact('organisation','role','id','role_ids'));
