<?php
	...
	class AjaxController extends Controller
	{
		...
		*/
		...
		public function EmployeeDetailsNew(){
			//echo "helooo"; die;
		...
		public function ViewEmpLeaveData(Request $request){
			...
			if(!empty($get_flow_data[0])){
				...
				$data_approval = DB::select("SELECT * FROM `leave_authorities` WHERE flow_id = $flow_id AND office_id=$officeId");
			...
			if($approval_count > 0 && $data[0]->status=='Pending'){
				if($forwarded_by ){
					...
					if($key > 0){
						$new_data_approval_array = $data_approval[$key-1];
						}else{
					...
					$new_data_approval_array = $data_approval[$approval_count-1];					
					$leave_status_at_user = $new_data_approval_array->user_id;									
				}
		...
		public function WithdrawLeaves(Request $request){
			...
			$data = DB::select("SELECT a.user_id, a.forwarded_by, a.start_date, a.end_date, a.duration, a.time_duration, b.name, b.email, b.mobile FROM `leaves` as a INNER JOIN users as b on a.user_id=b.id WHERE a.id=$leave_id");
			...
			if(!empty($data[0])){
				...
				//$update_leave_sql = "UPDATE `leaves` set duration = '".$total_leave_take_by_user."' where id=$leave_id";
