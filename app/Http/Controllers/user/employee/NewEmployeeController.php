	...
	class NewEmployeeController extends Controller
	{
		...
		*/
		...
		public function EmployeeDetails(){
			...
			return view('user.employeeNew.employee_details',compact('organisation'));
		}
		...
		public function addLeave(request $request){
			...
			if(!empty($request->user_id)){
				...
				$addLeaves->time_duration = $request->time_duration;
		...
		public function listLeave(){
			...
			$organisation = Organisation::where(['user_id'=>$user_id])->first();
			$office = OfficeMaster::select('id','office_name')->where('orgnization_id',$user_id)->get();
			$data = DB::select("SELECT a.name as leave_type,b.id, b.user_id, b.start_date,b.end_date,b.duration,b.time_duration,b.reason_for_leav_comp,b.created_at,b.status,c.name,c.mobile FROM `leave_types` as a INNER JOIN leaves as b on a.id=b.leave_type INNER JOIN users as c on c.id=b.user_id  WHERE a.orgnization_id=$user_id order by b.id DESC" );
			return view('organization.employee.view_employee_leave_details',compact('organisation','office','data'));
		}
		...
		public function DeleteFlow($id){   
			...
			///////////////////// Delete all approval flow when flow master is deleted //////////////////
			ApprovalFlow::where('flow_id',$id)->where('orgnization_id',$user_id)->delete();
		...
		public function viewEmployeeRemainingLeavesList(){
			$user_id = Auth::user()->id;
			$organisation = Organisation::where(['user_id'=>$user_id])->first(); 
			//$leave_details = DB::select("SELECT a.user_id as id,CONCAT(a.first_name,' ',a.last_name) as name FROM `emp_details` as a LEFT JOIN leaves as b on a.user_id=b.user_id INNER JOIN users as c on c.id=a.user_id WHERE a.created_by=$user_id AND c.type=2 GROUP by a.id ORDER BY name ASC;");
			/* $leave_details = DB::select(		
				"SELECT a.id as user_id,a.name, c.`emp_type`, d.`name` as leave_name, e.total_leaves as total_remaining_leaves FROM `users` as a 
				INNER JOIN employee_infos as b on a.id=b.user_id 
				INNER JOIN emp_types as c on c.orgnization_id=b.organisation_id AND c.office_id=b.office_id
				INNER JOIN leave_types as d ON d.orgnization_id=b.organisation_id AND d.office_id=b.office_id AND c.id=d.emp_type
				LEFT OUTER JOIN remaining_leaves as e ON e.leave_type_id=d.`id` AND e.office_id=d.office_id AND d.emp_type=e.employee_type
			WHERE b.employee_code is NOT null order by a.id asc"); */
			
			
			
			$leave_details = DB::select("SELECT a.*, b.`name`, c.employee_code, d.`emp_type`,e.id as leave_type_number, e.name as leave_name FROM `remaining_leaves` as a 
			INNER JOIN users as b on a.user_id=b.id 
			INNER JOIN employee_infos as c on c.user_id=b.id		
			INNER JOIN emp_types as d on d.orgnization_id=a.organization_id AND d.office_id=a.office_id
			INNER JOIN leave_types as e ON e.id=a.leave_type_id
			WHERE c.employee_code is NOT null");		 
			
			return view('organization.employee.view_employee_leave_list_data',compact('organisation','leave_details'));
		}
		
		
		public function getShortLeaveCount(){
		
			$user_id = Auth::user()->id;
			$organisation = Organisation::where(['user_id'=>$user_id])->first();
			
			$getShortLeaveData = DB::select("SELECT id, name from leave_types where name LIKE 'short leave%' AND orgnization_id=$organisation->user_id");
			
			//echo "<pre>".'getShortLeaveData--';print_r($getShortLeaveData);echo "</pre>";
			$leaves_array = []; 
			
			if(!empty($getShortLeaveData)){
				foreach($getShortLeaveData as $getShortLeaveDetails){					
					$leaves_array[] = $getShortLeaveDetails->id;					
				}			
			}
			//echo "<pre>".'leaves_array--';print_r($leaves_array);echo "</pre>";
			$ids = implode(',', $leaves_array);	

			//echo "SELECT lvs.id, count(lvs.id) as total_leave, lvs.start_date, lvs.user_id, lvs.office_id, lvs.leave_type, us.name as user_name, us.email, us.mobile, us.organisation_id, lt.name as leave_name, lt.emp_type FROM `leaves` as lvs INNER JOIN users as us ON lvs.user_id=us.id INNER JOIN `leave_types` as lt on lt.id=lvs.leave_type where lvs.status='Approved' AND lvs.leave_type IN ($ids) AND month(lvs.end_date)=month(now())-1 group by lvs.user_id HAVING COUNT(lvs.id) > 0";			
			$short_leave_count = DB::select("SELECT lvs.id, count(lvs.id) as total_leave, lvs.start_date, lvs.user_id, lvs.office_id, lvs.leave_type, us.name as user_name, us.email, us.mobile, us.organisation_id, lt.name as leave_name, lt.emp_type FROM `leaves` as lvs INNER JOIN users as us ON lvs.user_id=us.id INNER JOIN `leave_types` as lt on lt.id=lvs.leave_type where lvs.status='Approved' AND lvs.leave_type IN ($ids) AND month(lvs.end_date)=month(now())-1 group by lvs.user_id HAVING COUNT(lvs.id) > 0");
			
			if(!empty($short_leave_count)){
				foreach($short_leave_count as $short_leave_count_data){					
					$group_short_leave_by_user[] = $short_leave_count_data;					
				}			
			}			
			 
			return view('organization.employee.view_employee_monthly_short_leave',compact('organisation','short_leave_count')); 
		}
